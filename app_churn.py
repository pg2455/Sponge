...
def calculateTotalForUser(item_list,namespace):
    ...
    tips = merchant_info.find_one({"_id": ObjectId(merchant_id)})['tip']
...
def setItems():
    ...
    if order_id:
        ...
        for item in item_list['selected_items']:
            for x,order_no in enumerate(order['selected_items']):
                if order_no['name'] == item['name']:
                    order['selected_items'][x]['claimed'] += item['claimed']
                    order['selected_item'][x]['users'].append(user_id)
        ...
        data['transactions'][order_id]['payment_status']['users'][user_id] ={"selected_items": item_list['items']}

        total =  calculateTotalForUser(order['payment_status']['users'][user_id],namespace)
